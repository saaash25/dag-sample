...
//..........................................................................
exports.GetStandardizedPayload = () => {
    try {
...
ChildCheck = async (nodeList, node, payload) => {
    try {
        if (node.func) {
            const func = await CreateFunction(node.func)
            returnObj = await ExecuteFunctions(func, payload)
        }
        //For Branch Node there is no child nodes availble
        //For Nodes having child nodes , no branches available
        if (node.childNodes.length) {
            const childNode = await GetChild(nodeList, node.childNodes[0].child)
            await ChildCheck(nodeList, childNode[0], payload)
        }
        else if (node.branches.length) {
            const branch = await ChooseCorrectBranch(node.branches, payload)
            if(branch){
                const childNode = await GetChild(nodeList, branch.childNodes[0].child)
                await ChildCheck(nodeList, childNode[0], payload)
            }
            return returnObj
        }
...
GetChild = async (nodeList, childNodeId) => {
    try {
...
ChooseCorrectBranch = async (branches, payload) => {
    try {
        const branchCount = branches.length
        let i = 0
        let condition = false
        while (condition === false) {
            const func = CreateFunction(branches[i].func)
            const result = await ExecuteFunctions(func, payload)
            if (result) {
                condition = result
                return branches[i]
            }
